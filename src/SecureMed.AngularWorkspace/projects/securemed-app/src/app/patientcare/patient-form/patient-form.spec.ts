...
import { provideEventPlugins } from '@securemed-app/shared/event-managers';
import type { CreatePatientCommand } from '@securemed-app/patientcare/models';
...
it('submits form with complete data when valid', async () => {
	const { user, mockRequest, patientCreatedSpy: onSubmittedSpy } = await setup();
	...
	const expectedData: CreatePatientCommand = {
		firstName: 'John',
		lastName: 'Doe',
        insz: '456789012345',
		address: {
...
it('displays error message when API request fails', async () => {
	...
	await mockRequest({ title: 'Patient creation failed' }, { status: 500, statusText: 'Bad Request' });

	expect(await screen.findByText('Patient creation failed')).toBeInTheDocument();
	expect(screen.queryByRole('button', { name: /create patient/i })).toBeEnabled();
});

async function setup() {
	const patientCreatedSpy = vi.fn();
	const user = userEvent.setup();

	const { fixture } = await render(PatientForm, {
		bindings: [outputBinding('patientCreated', patientCreatedSpy)],

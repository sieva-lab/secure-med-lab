...
import { type PatientDetailsResponse } from '@securemed-app/patientcare/models';
import { generateUuid } from '@securemed-app/shared/functions';
import { inputBinding, signal } from '@angular/core';
import { stripUtilAttributes } from '@securemed-app/test/test-utils';
import PatientDetails from './patient-details';

it('renders patient details when data is loaded', async () => {
	const { mockRequest } = await setup();

	expect(screen.queryByText(/Loading patient/i)).toBeInTheDocument();

	await mockRequest(patientDetails);
...
it('does not display address info when no addresses are present', async () => {
	const patientWithNoAddresses: PatientDetailsResponse = {
		...patientDetails,
		billingAddresses: [],
		shippingAddresses: [],
	};
	...
	await mockRequest(patientWithNoAddresses);
...
async function setup() {
	...
	const patientId = signal(generateUuid());
	const { fixture } = await render(PatientDetails, {
		bindings: [inputBinding('patientId', patientId)],
	});
	const httpMock = TestBed.inject(HttpTestingController);
	return {
		mockRequest: async (response: object, opts?: object) => {
			const request = httpMock.expectOne(`/api/patients/${patientId()}`);
...
const patientDetails: PatientDetailsResponse = {
	id: generateUuid(),
	firstName: 'John',
	lastName: 'Doe',
	address: { id: generateUuid(), street: '123 Billing St', city: 'Bill City', zipCode: '12345' },
